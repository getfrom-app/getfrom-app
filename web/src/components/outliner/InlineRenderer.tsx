...
export function renderInlineToHtml(text: string, highlight?: string, forcedBlock?: 'bullet' | 'h1' | 'h2' | 'h3'): string {
  ...
  let html = esc(content)
    ...
    .replace(/(?<!\w)@([\w\u00C0-\u024F][\w\u00C0-\u024F\s\-]*)/g, (_match, slug) => {
      ...
      return `<span class="context-inline" data-slug="${sl}" style="display:inline-flex;align-items:center;gap:3px;background:${hex}18;color:${hex};border:1px solid ${hex}40;border-radius:999px;font-size:0.8em;font-weight:500;padding:0 4px 0 5px;cursor:pointer">${esc(part)}<button class="ctx-chip-remove" data-slug="${sl}" title="${i18n.t('common.removeContext')}" style="background:none;border:none;cursor:pointer;color:${hex};opacity:0.6;padding:0;font-size:1em;line-height:1;display:flex">×</button></span>`
    })
