...
export default function V2ConversationView({ sessionId, onOpenNode, onSelectCtx }: Props) {
  ...
  const detachFromConversation = (nodeId: string) => {
    ...
    window.dispatchEvent(new CustomEvent('from:toast', { detail: { message: t('v2.detachedFromConversation', 'Quitado de la conversación (sigue en Fromly)'), type: 'success' } }))
  }

  return (
    <div>
      {/* Añadir la conversación a un contexto (buscador con crear, como en v1). */}
      <div className="v2-section-label" style={{ padding: '0 0 6px' }}>{t('v2.conversation', 'Conversación')}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        ...
        <div className="v2-ctxpick-wrap" ref={pickerWrap}>
          <button className="v2-ctx-add-btn" onClick={() => setPickerOpen(o => !o)}>＋ {t('v2.addToContextEllipsis', 'Añadir a contexto…')}</button>
          {pickerOpen && (
      ...
      <div className="v2-section-label" style={{ padding: '16px 0 4px' }}>{t('v2.elementsCount', 'Elementos ({{count}})', { count: elements.length })}</div>
          ...
          {thumbs.map(({ node: n, kind }) => {
            ...
            return (
              <div className="v2-thumb" key={n.id} title={n.text || ''} role="button" tabIndex={0}
                ...
                <button className="v2-thumb-detach" title={t('v2.removeFromThisConversation', 'Quitar de esta conversación')}
